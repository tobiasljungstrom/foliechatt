...
var App = React.createClass({
    getInitialState: function() {
        return {sessionToken: null, roomList: [], loggedInUser: null};
    },
    ...
    render: function() {
        ...
        if (this.state.roomList.length > 0) {
            this.state.roomList.forEach((room, index) => {
                chatRooms[index] = <ChatRoom key={index} sessionToken={this.state.sessionToken} baseUrl={this.state.baseUrl} loggedInUser={this.state.loggedInUser} messages={this.state.roomList[index].messages} users={this.state.roomList[index].users} roomId={this.state.roomList[index].roomId} cryptoHelper={this.state.roomList[index].cryptoHelper} leaveChatRoom={this.leaveChatRoom} updateChat={this.updateChat} updateUsers={this.updateUsers}/>;
            });
        }
